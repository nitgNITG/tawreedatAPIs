...
// Zod schema for faqs reorder
const faqsReorderSchema = z.object({
  faqs: z
    .array(
      z.object({
...
router.post("/", authorization, async (req, res) => {
  ...
  try {
    ...
    // Validate input
    const parsed = faqsReorderSchema.safeParse(req.body);
    ...
    // Build SQL
    const caseSql = faqs
      .map((faq) => `WHEN ${faq.id} THEN ${faq.sortId}`)
      .join(" ");
    ...
    await revalidateDashboard("faqs");
  } catch (error) {
    console.error(error);
    res.status(500).json({
