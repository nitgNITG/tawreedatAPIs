...
// Zod schema for ads reorder
const adsReorderSchema = z.object({
  ads: z
    .array(
      z.object({
...
router.post("/", authorization, async (req, res) => {
  ...
  try {
    ...
    // Validate input
    const parsed = adsReorderSchema.safeParse(req.body);
    ...
    // Build SQL
    const caseSql = ads
      .map((ad) => `WHEN ${ad.id} THEN ${ad.sortId}`)
      .join(" ");
    const idsSql = ads.map((ad) => ad.id).join(",");
