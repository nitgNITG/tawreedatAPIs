...
// Zod schema for articles reorder
const articlesReorderSchema = z.object({
  articles: z
    .array(
      z.object({
...
router.post("/", authorization, async (req, res) => {
  ...
  try {
    ...
    // Validate input
    const parsed = articlesReorderSchema.safeParse(req.body);
    ...
    await revalidateDashboard("articles");
  } catch (error) {
    console.error(error);
    res.status(500).json({
